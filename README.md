...
- **Нагадання**:
  - Если задача (своя или делегированная) остается в статусе `pending` дольше `NOT_STARTED_REMINDER_MINUTES` (по умолчанию 30 мин), бот отправляет напоминание "не взял в работу".
  - После дедлайна, пока задача не `done`/`cancelled`, бот отправляет повторное напоминание каждые `OVERDUE_REPEAT_HOURS` (по умолчанию 1 час) — исполнителю и владельцу для делегированных задач, только владельцу для собственных.
...
- `/add_executor <имя>` — отправляет ссылку-инвайт; исполнитель переходит и регистрируется сам (без ручного ID)
- `/add_executor <telegram_id> <имя>` — добавить исполнителя вручную, если ID уже известен
